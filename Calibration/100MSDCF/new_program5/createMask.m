function [obj] = createMask(RGB, bw, hist_size, output_minima_mid, Threshold, band, op_th)
warning off
split = sum(bw);
split = find(split~=0);
x = []; x(1,1) = split(1); k = 1;
...
x(k,2) = split(end); y = []; bw_ch = bw;
for i = 1:size(x,1)
    bw_cut = bw(:,x(i,1):x(i,2),:);
    bw_ch(:,x(i,1):x(i,2),:) = bwconvhull(bw_cut);
end
...
x(k,2) = split(end); y = [];
for i = 1:size(x,1)
    bw_cut = BW(:,x(i,1):x(i,2),:) | bw_ch(:,x(i,1):x(i,2),:);
    bw_cut = sum(bw_cut');
    idx = find(bw_cut~=0);
    y = [y; min(idx),max(idx)];
end
