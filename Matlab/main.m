...
for count_i=1:size(angles,2)
    for count_k=1:size(cameradist,2)
        ...
        headPosy_range = 20:2:round(min([exPointRight(2),exPointRight(2)]));
        
        for count_j=1:size(headPosx_range,2)
            for count_l=1:size(headPosy_range,2)
                if exist(['D:/Plot/plot',num2str(count_res),'.png'], 'file')
                    toplot = false;
                else
                    toplot = true;
                end
...
%                 headPosy = 20;              %cm
                headPosx = headPosx_range(count_j)*pivot2pivot; %cm % x0,y0 ellipse centre coordinates
                headPosy = headPosy_range(count_l);             %cm
                ...
                Results(count_res,1) = {angles(count_i)};
                Results(count_res,2) = {cameradist(count_k)};
                Results(count_res,3) = {headPosx_range(count_j)*pivot2pivot};
                ...
                assert(size(Results,1)==count_res);
                count_res = count_res +1;
                if it == 0
                    tic
                end
                it = it + 1;
                if mod(count_res,100) ==0
                    disp(['Iteration: ',num2str(count_res),' N° Com: ', num2str(num_c), ' N° Part: ',num2str(num_p/2), ' Angle: ' , num2str(angles(count_i))]);
                    toc
                    it = 0;
                end
            end
