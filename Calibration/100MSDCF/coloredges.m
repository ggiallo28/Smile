...
%
	...
	rx = imfilter(imadjust(im(:,:,1)), xfilter);
	gx = imfilter(imadjust(im(:,:,2)), xfilter);
	bx = imfilter(imadjust(im(:,:,3)), xfilter);
	cx = imfilter(imadjust(cymk(:,:,1)), xfilter);
    yx = imfilter(imadjust(cymk(:,:,2)), xfilter);
    mx = imfilter(imadjust(cymk(:,:,3)), xfilter);
    kx = imfilter(imadjust(cymk(:,:,4)), xfilter);
    
	ry = imfilter(imadjust(im(:,:,1)), yfilter);
	gy = imfilter(imadjust(im(:,:,2)), yfilter);
	by = imfilter(imadjust(im(:,:,3)), yfilter);
    cy = imfilter(imadjust(cymk(:,:,1)), yfilter);
    yy = imfilter(imadjust(cymk(:,:,2)), yfilter);
    my = imfilter(imadjust(cymk(:,:,3)), yfilter);
    ky = imfilter(imadjust(cymk(:,:,4)), yfilter);
	
	Jx = rx.^2 + gx.^2 + bx.^2 + cx.^2 + yx.^2 + mx.^2 + kx.^2;
	Jy = ry.^2 + gy.^2 + by.^2 + cy.^2 + yy.^2 + my.^2 + ky.^2;
	Jxy = rx.*ry + gx.*gy + bx.*by + cx.*cy + yx.*yy + mx.*my + kx.*ky;
