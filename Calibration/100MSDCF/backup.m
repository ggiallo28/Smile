...
for i = 1:size(x,1)
    ...
    for k=1:5
        for j=1:5
            scatter(centers(k,j,1,i),centers(k,j,2,i))
        end
    end
    pause
    close all;
...
for i = 1:size(x,1)
    ...
    for k=1:size(centers,2)
        ...
        if(sum(x_axis)>0 && k<=j)
            ...
            val = min(line2);
            idx = find(line2==val);
            centers(2,j,1,i) = xq_axis(idx);
            line2 = abs(centers(4,k,2,i)-line);
            val = min(line2);
            idx = find(line2==val);
            centers(4,j,1,i) = xq_axis(idx);
        end
        ...
        if(sum(x_axis)>0  && k<=j)
            ...
            val = min(line2);
            idx = find(line2==val);
            centers(1,j,1,i) = xq_axis(idx);
            
            line2 = abs(centers(3,k,2,i)-line);
            val = min(line2);
            idx = find(line2==val);
            centers(3,j,1,i) = xq_axis(idx);
            
            line2 = abs(centers(5,k,2,i)-line);
            val = min(line2);
            idx = find(line2==val);
            centers(5,j,1,i) = xq_axis(idx);
        end
        j = j-1;
    end
end
