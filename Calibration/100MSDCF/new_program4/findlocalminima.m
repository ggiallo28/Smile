...
%
%-------------------------------------------------------------%
function [output_peak, output_minima_low, output_minima_high , output_mid, hist_size] = findlocalminima(I,mpd,windowSize,refine, isPeriodic)
...
if(size(output_peak,2)==5)
    ...
    output_mid(3) = output_peak(3)+0.5*(output_peak(4)-output_peak(3));
    output_mid(4) = output_peak(4)+0.5*(output_peak(5)-output_peak(4));
end
if(size(output_peak,2)==4)
        output_mid(1) = output_peak(1)+0.5*(output_peak(2)-output_peak(1));
        output_mid(2) = output_peak(2)+0.5*(output_peak(3)-output_peak(2));
        output_mid(3) = output_peak(3)+0.5*(output_peak(4)-output_peak(3));
        output_mid(4) = mod(output_peak(4)+0.5*abs((256+output_peak(1))-output_peak(4)),256);
