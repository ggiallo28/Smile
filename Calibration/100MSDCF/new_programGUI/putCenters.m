...
toplot = true & ~Container.isGUI;
    ...
    if Container.isGUI && false
        axes_gui = Container.app.UIAxes3_2;
        cla(axes_gui);
        imshow(obj.color_mask,'Parent',axes_gui);hold(axes_gui,'on');
        tf_ = [1:0.01:size(obj.color_mask,2)];
        yf_ = feval(lineT, tf_);
        tf_(yf_<1 | yf_>size(obj.color_mask,1)) = [];
        yf_(yf_<1 | yf_>size(obj.color_mask,1)) = [];
        ...
        yf_(yf_<1 | yf_>size(obj.color_mask,1)) = [];
        plot(axes_gui, tf_,yf_,'r'); l1 = legend(); set(l1,'visible','off');
        scatter(axes_gui, obj.chess(index).center_x(j,:),obj.chess(index).center_y(j,:))
        l1 = legend(); set(l1,'visible','off');
        drawnow;
    end
end
